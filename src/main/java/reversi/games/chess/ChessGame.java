...
public final class ChessGame implements ChessSession {
    ...
    public String cellDisplay(Position p) {
        return String.valueOf(pieceAt(p));
    }
    ...
    private Map<Disc, Integer> counts() {
    ...
    private ActionResult tryCastle(Position from, Position to, char king) {
        CastlePlan plan = castlePlan(from, to, king);
        if (!plan.valid()) {
        ...
        setPiece(from, EMPTY);
        setPiece(to, king);
        setPiece(new Position(plan.row(), plan.rookCol()), EMPTY);
        setPiece(new Position(plan.row(), plan.rookTargetCol()), rook);
        markMoved(from, king);
        markCastleRookMoved(side, plan.kingSide());
        clearEnPassant();
        current = current.opposite();
        var endResult = checkGameEnd(current);
        if (endResult != null) {
        ...
        moveCount++;
        lastMessage = plan.kingSide() ? "王侧易位完成" : "后侧易位完成";
        return ActionResult.OK;
    }
    ...
    private void markCastleRookMoved(Disc side, boolean kingSide) {
        if (side == Disc.BLACK) {
            if (kingSide) {
    ...
    private boolean canCastle(Position from, Position to, char king) {
        return castlePlan(from, to, king).valid();
    }

    private CastlePlan castlePlan(Position from, Position to, char king) {
        Disc side = sideOf(king);
        int row = side == Disc.BLACK ? 0 : 7;
        if (from.row() != row || from.col() != 4 || to.row() != row) {
            return CastlePlan.invalid("王车易位条件不满足");
        }
        if ((side == Disc.BLACK && blackKingMoved) || (side == Disc.WHITE && whiteKingMoved)) {
            return CastlePlan.invalid("王车易位条件不满足: 王已经移动过");
        }
        if (isKingInCheck(side)) {
            return CastlePlan.invalid("王车易位条件不满足: 王正被将军");
        }
        ...
        if (pieceAt(new Position(row, rookCol)) != rook || rookMoved(side, kingSide)) {
            return CastlePlan.invalid("王车易位条件不满足: 车已经移动过或不在原位");
        }
        for (int col : emptyCols) {
            if (pieces[row][col] != EMPTY) {
                return CastlePlan.invalid("王车易位条件不满足: 路径上有棋子");
            }
        }
        for (int col : safeCols) {
            if (isSquareAttacked(new Position(row, col), side.opposite())) {
                return CastlePlan.invalid("王车易位条件不满足: 王经过或到达的格子被攻击");
            }
        }
        return CastlePlan.valid(row, kingSide, rookCol, rookTargetCol);
    }
    ...
    private record CastlePlan(boolean valid, String message, int row,
                              boolean kingSide, int rookCol, int rookTargetCol) {
        static CastlePlan valid(int row, boolean kingSide, int rookCol, int rookTargetCol) {
            return new CastlePlan(true, "", row, kingSide, rookCol, rookTargetCol);
        }

        static CastlePlan invalid(String message) {
            return new CastlePlan(false, message, 0, false, 0, 0);
        }
    }
}
